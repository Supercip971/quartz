...
public:
    ...
    static Result<Shader> fromData(vk::Device &dev, std::vector<uint8_t> const &data, ShaderKind kind) {
        Shader shader(kind);
        shader._data = data;

        vk::ShaderModuleCreateInfo createInfo = vk::ShaderModuleCreateInfo()
                                                    .setCodeSize(data.size())
                                                    .setPCode(reinterpret_cast<uint32_t const *>(data.data()));

        vkTry$(dev.createShaderModule(&createInfo, nullptr, &shader._module));

        return Result<Shader>::ok(std::move(shader));
    }
    static Result<Shader> fromFile(vk::Device &dev, std::string const &path, ShaderKind kind) {
